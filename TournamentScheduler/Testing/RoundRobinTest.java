...
public class RoundRobinTest {
	...
	public void testRoundRobin() {
		...
		assertThat(schedule.elementAt(0).toString(), equalTo("Robbin' Round match 1: P3 vs. P1 at C1 (V1), 10/21/11 2:30 PM - 3:00 PM"));
		assertThat(schedule.elementAt(1).toString(), equalTo("Robbin' Round match 2: P3 vs. P2 at C1 (V1), 10/21/11 1:30 PM - 2:00 PM"));
		assertThat(schedule.elementAt(2).toString(), equalTo("Robbin' Round match 3: P1 vs. P2 at C2 (V2), 10/21/11 5:30 PM - 6:00 PM"));
		...
		assertThat(robin.TrySchedule(schedule), equalTo(true));
		assertThat(schedule.size(), equalTo(3));
		assertThat(schedule.elementAt(0).toString(), equalTo("SLG Tournament match 1: P3 vs. P1 at C1 (V1), 10/21/11 12:40 PM - 2:00 PM"));
		assertThat(schedule.elementAt(1).toString(), equalTo("SLG Tournament match 2: P3 vs. P2 at C1 (V1), 10/21/11 10:00 AM - 11:20 AM"));
		assertThat(schedule.elementAt(2).toString(), equalTo("SLG Tournament match 3: P1 vs. P2 at C1 (V1), 10/21/11 11:20 AM - 12:40 PM"));
		
		// Try again with longer matches to force a player to be dropped.
		c1 = new Court(1, "C1", "V1", threeToFive, tenToThree);
		c2 = new Court(2, "C2", "V2", tenToThree, fiveToSix);
		courts = new CourtManager(c1, c2);
		...
		p2 = new Player(5, "P2", tenToThree, fiveToSix);
		p3 = new Player(6, "P3", tenToThree);
		robin = new RoundRobin(2, "Fail Tournament", 120, courts, p1, p2, p3);
		robin.LinkTo(robin);
		schedule = new Vector<Match>();
		assertThat(robin.TrySchedule(schedule), equalTo(true));
